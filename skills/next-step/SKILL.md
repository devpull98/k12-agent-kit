...
# Parallel fan-out / join patterns

Dùng khi plan có `parallel_safe: true` hoặc user muốn chạy song song:

| Pattern | Khi dùng | Cách thực hiện |
|---------|----------|---------------|
| **Fan-out** | Nhiều task độc lập sau 1 skill | List tasks: "Bạn có thể chạy song song: [A, B, C] — không phụ thuộc nhau" |
| **Fan-in (join)** | Tất cả nhánh phải xong trước bước tiếp | "Chỉ chuyển sang [next-skill] khi cả [A] và [B] đều pass" |
| **Sequential gate** | Nhánh phụ thuộc nhánh kia | "Chạy A trước, đợi pass → mới chạy B" |

**Ví dụ fan-out sau `writing-plans`:**
```
Các tasks sau có thể chạy song song (parallel_safe: true):
- Task 1: implement LoginService  → tdd
- Task 2: implement UserRepository → tdd
Join point: cả 2 pass → code-review
```

**Ví dụ fan-in trước `shipping`:**
```
Chỉ ship khi tất cả đều pass:
- [ ] code-review: APPROVE
- [ ] qc-automation: qc_status = pass
- [ ] trace-validation: no GAP blocker
```
